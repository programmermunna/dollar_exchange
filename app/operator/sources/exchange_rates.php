                ...
                <div class="card-body">
                     ...
                     if(isset($CEAction) && $CEAction == "new") {
                        ...
                        if(empty($gateway_from) or empty($gateway_to)) {
                            ...
                            $query = $db->query("SELECT * FROM ce_rates WHERE gateway_from='$gateway_from' and gateway_to='$gateway_to' ORDER BY id DESC LIMIt 1");
                            $row = $query->fetch_assoc();
                            echo success("Your rate was added successfully.");
                            // update operator activity start
							$activity_time = time();
							$activity_ip = $_SERVER['REMOTE_ADDR'];
							$update = $db->query("INSERT ce_operators_activity (oid,activity_type,activity_id,activity_value,ip,created) VALUES ('$_SESSION[ce_operator_uid]','new_rate','$row[id]','','$activity_ip','$activity_time')");
                ...
                <div class="card-body">
                     ...
                     if(isset($CEAction) && $CEAction == "save") {
                        ...
                        if($percentage_fee !== "allow" && empty($rate_from) or $percentage_fee !== "allow" && empty($rate_to)) {
							...
							$activity_time = time();
							$activity_ip = $_SERVER['REMOTE_ADDR'];
							$update = $db->query("INSERT ce_operators_activity (oid,activity_type,activity_id,activity_value,ip,created) VALUES ('$_SESSION[ce_operator_uid]','update_rate','$row[id]','','$activity_ip','$activity_time')");
							// update operator activity end
                        }
                     }
                     ?>
