...
if($op['can_manage_support_tickets'] !== "1") {
    header("Location: ./");
}
...
if($b == "view") {
                    ...
                    if(isset($CEAction) && $CEAction == "post") {
                        ...
                        if(empty($message)) {
                            ...
                            if(empty($row['served_by'])) {
                                $update = $db->query("UPDATE ce_tickets SET served_level='2',served_by='$_SESSION[ce_operator_uid]' WHERE id='$row[id]'");
                                // update operator activity start
                        $activity_time = time();
                        $activity_ip = $_SERVER['REMOTE_ADDR'];
                        $update = $db->query("INSERT ce_operators_activity (oid,activity_type,activity_id,activity_value,ip,created) VALUES ('$_SESSION[ce_operator_uid]','serve_ticket','$row[id]','','$activity_ip','$activity_time')");
                        ...
                        $activity_time = time();
                        $activity_ip = $_SERVER['REMOTE_ADDR'];
                        $update = $db->query("INSERT ce_operators_activity (oid,activity_type,activity_id,activity_value,ip,created) VALUES ('$_SESSION[ce_operator_uid]','reply_ticket','$row[id]','','$activity_ip','$activity_time')");
                        // update operator activity end
    ...
    $activity_time = time();
    $activity_ip = $_SERVER['REMOTE_ADDR'];
    $update = $db->query("INSERT ce_operators_activity (oid,activity_type,activity_id,activity_value,ip,created) VALUES ('$_SESSION[ce_operator_uid]','solve_ticket','$row[id]','','$activity_ip','$activity_time')");
    // update operator activity end
    if(empty($served_by)) {
    ...
    $activity_time = time();
    $activity_ip = $_SERVER['REMOTE_ADDR'];
    $update = $db->query("INSERT ce_operators_activity (oid,activity_type,activity_id,activity_value,ip,created) VALUES ('$_SESSION[ce_operator_uid]','close_ticket','$row[id]','','$activity_ip','$activity_time')");
    // update operator activity end
    if(empty($served_by)) {
        $update = $db->query("UPDATE ce_tickets SET served_by='$_SESSION[ce_admin_uid]' WHERE id='$row[id]'");  
    }
