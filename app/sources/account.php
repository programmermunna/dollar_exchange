...
if($b == "dashboard") {
    ...
    if($E_Query->num_rows>0) {
        while($e_row = $E_Query->fetch_assoc()) {
            ...
            $etpl->set("date",date("d/m/y h:ia",$e_row['created']));
            ...
            if($MessagesQuery->num_rows>0) {
                while($msg = $MessagesQuery->fetch_assoc()) {
                    if($msg['author']>0) {
                        ...
                        $mtpl->set("date",date("d/m/y h:ia",$msg['created']));
	...
	if($E_Query->num_rows>0) {
        while($e_row = $E_Query->fetch_assoc()) {
            ...
            $etpl->set("url",$settings['url']);
            if($e_row['updated']>0) {
                $date = date("d/m/y h:ia",$e_row['updated']);
            } else {
                $date = date("d/m/y h:ia",$e_row['created']);
            }
            if($e_row['status'] == "9") {
	...
	if($E_Query->num_rows>0) {
        while($e_row = $E_Query->fetch_assoc()) {
            ...
            $etpl->set("type",$type);
            $etpl->set("date",date("d/m/y h:ia",$e_row['posted']));
            if($e_row['status'] == "1") {
    ...
    if($WithdrawalsQuery->num_rows>0) {
        while($w = $WithdrawalsQuery->fetch_assoc()) {
            ...
            $wtpl->set("date",date("d/m/y h:ia",$w['requested_on']));
