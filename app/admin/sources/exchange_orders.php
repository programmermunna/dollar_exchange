            ...
            <div class="col-md-12 col-lg-12">
                ...
                if(isset($CEAction) && $CEAction == "update") {
                    ...
                    if($status == "4") { 
                        ...
                        // Custom Codes by Rahat Khan for updating Gateway Reserve
                        // Get old reserve
                        $gatewayid = $row['gateway_receive'];
                        
                        $gatewayquery = $db->query("SELECT * FROM ce_gateways WHERE id='$gatewayid'");
                        $gatewayrow = $gatewayquery->fetch_assoc();
                        //print_r($gatewayrow);
                        
                        $oldreserve = $gatewayrow['reserve'];
                        //print_r($oldreserve);
                         
                        
                        $deletedamount = $row['amount_receive'];
                        //print_r($deletedamount);
                        
                        // //Set New reserve
                        $newreserve = $oldreserve - $deletedamount;
                        //print_r($newreserve);
                        
                        $update = $db->query("UPDATE ce_gateways SET reserve=$newreserve WHERE id='$gatewayid'");
